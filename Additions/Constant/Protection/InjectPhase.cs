...
namespace Confuser.Protections.Constants {
	internal class InjectPhase : ProtectionPhase {
		...
		public override string Name {
			get { return Resources.InjectPhase_Name; }
		}
		...
		protected override void Execute(ConfuserContext context, ProtectionParameters parameters) {
			if (parameters.Targets.Any()) {
				...
				// Extract parameters
				moduleCtx.Mode = parameters.GetParameter(context, context.CurrentModule, "mode", Mode.x86);
		...
		void InjectHelpers(ConfuserContext context, ICompressionService compression, IRuntimeService rt, CEContext moduleCtx) {
			IEnumerable<IDnlibDef> members = InjectHelper.Inject(rt.GetRuntimeType("Confuser.Runtime.Constant2"), context.CurrentModule.GlobalType, context.CurrentModule);
			...
			MethodDef decoder = rt.GetRuntimeType("Confuser.Runtime.Constant2").FindMethod("Get");
			...
			for (int i = 0; i < moduleCtx.DecoderCount; i++) {
				...
				for (int j = 0; j < decoderInst.Body.Instructions.Count; j++) {
					...
					if (instr.OpCode == OpCodes.Call &&
					    method.DeclaringType.Name == "Mutation2" &&
					    method.Name == "Value") {
						decoderInst.Body.Instructions[j] = Instruction.Create(OpCodes.Sizeof, new GenericMVar(0).ToTypeDefOrRef());
					         ...
					         method.DeclaringType.Name == "Constant2") {
		...
		void MutateInitializer(CEContext moduleCtx, MethodDef decomp) {
			...
			for (int i = 0; i < instrs.Count; i++) {
				...
				var method = instr.Operand as IMethod;
				if (instr.OpCode == OpCodes.Call) {
					if (method.DeclaringType.Name == "Mutation2" &&
					    method.Name == "Crypt") {
					...
					else if (method.DeclaringType.Name == "Lzma2" &&
