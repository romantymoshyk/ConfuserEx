...
namespace Confuser.Runtime {
	internal static class Constant2 {
		static byte[] b;

		static void Initialize() {
			var l = (uint)Mutation2.KeyI0;
			uint[] q = Mutation2.Placeholder(new uint[Mutation2.KeyI0]);

			var k = new uint[0x10];
			var n = (uint)Mutation2.KeyI1;
			...
			b = Lzma2.Decompress(o);
		}

		static T Get<T>(uint id) {
            id = (uint)Mutation2.Placeholder((int)id);
            uint t = id >> 30;
            ...
            if (t == Mutation2.KeyI0)
            {
                int l = b[id++] | (b[id++] << 8) | (b[id++] << 16) | (b[id++] << 24);
                ret = (T)(object)string.Intern(Encoding.UTF8.GetString(b, (int)id, l));
            }
            // NOTE: Assume little-endian
            else if (t == Mutation2.KeyI1)
            {
                var v = new T[1];
                Buffer.BlockCopy(b, (int)id, v, 0, Mutation2.Value<int>());
                ret = v[0];
            }
            else if (t == Mutation2.KeyI2)
            {
