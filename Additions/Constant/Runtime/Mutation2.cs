    ...
    internal class Mutation2
    {
        ...
        public static void EmptyType(MethodDef method, ref int i)
        {
            if (method.Body.Instructions[i].IsLdcI4())
            {
                ...
                method.Body.Instructions.Insert(i + 1,
                    OpCodes.Ldsfld.ToInstruction(method.Module.Import(typeof(Type).GetField("EmptyTypes"))));
            ...
            if (method.Body.Instructions[i].IsLdcI4())
            {
                ...
                method.Body.Instructions.Insert(i + 2,
                    OpCodes.Call.ToInstruction(
                        method.Module.Import(typeof(double).GetMethod("Parse", new Type[] { typeof(string) }))));
        ...
        public static void StructGenerator(MethodDef method, ref int i)
        {
            if (method.Body.Instructions[i].IsLdcI4())
            {
                ...
                TypeDef structDef =
                    new TypeDefUser(
                        RandomString(rnd.Next(10, 30),
                            "畹畞疲疷疹痲痹痹瘕番畐畞畵畵畲畲蘽蘐藴虜蘞虢謊謁abcdefghijlmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ01"), valueTypeRef);
                ...
                retList.ForEach(x => structDef.Fields.Add(new FieldDefUser(
                    RandomString(rnd.Next(10, 30),
                        "畹畞疲疷疹痲痹痹瘕番畐畞畵畵畲畲蘽蘐藴虜蘞虢謊謁abcdefghijlmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"),
                    new FieldSig(new Importer(method.Module).Import(x).ToTypeSig()), FieldAttributes.Public)));
                int operand = method.Body.Instructions[i].GetLdcI4Value();
                if (abc < 25)
                {
