...
namespace Confuser.Protections.OpCodeProt {
	public class CallvirtPhase : ProtectionPhase {
		...
		public override string Name => Resources.CallvirtPhase_Name;

		protected override void Execute(ConfuserContext context, ProtectionParameters parameters) {
			var random = context.Registry.GetService<IRandomService>().GetRandomGenerator(Parent.FullId + ".Callvirt");
					...
					for (int i = 0; i < method.Body.Instructions.Count; i++) {
						if (method.Body.Instructions[i].OpCode == OpCodes.Callvirt) {
							if (method.Body.Instructions[i].Operand.ToString().ToLower().Contains("int32")) {
								if ((i - 1) > 0 && method.Body.Instructions[i - 1].IsLdloc()) {
